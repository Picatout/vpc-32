...
void initSD(void)
{
	...
	SPI2BRG = (mGetPeripheralClock() / (2 * 250000)) - 1;
...
void disableSD(void)
{
       ...
       PORTB |= SDCSEL; //digitalwrite(SDCSEL, HIGH);	// Deselected = SDCSEL high
       clockSPI();
}

void enableSD(void)
{
	PORTB &= ~ SDCSEL; //digitalwrite(SDCSEL, LOW);	// Selected = SDCSEL low
}
