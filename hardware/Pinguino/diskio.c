...
static volatile UINT16 Timer1, Timer2; /* 1000Hz decrement timer */

static UINT16 CardType;
...
static int rcvr_datablock( /* 1:OK, 0:Failed */
                          ...
                          UINT16 btr /* Byte count (must be multiple of 4) */
                          ) {
	PF_BYTE d;
	UINT16 tmr;
...
DRESULT disk_read(PF_BYTE drv, /* Physical drive nmuber (0) */
	...
	if (!(CardType & CT_BLOCK))
		sector *= BLK_SIZE; /* Convert to byte address if needed */

	if (count == 1) { /* Single block read */
		if ((send_cmd(CMD17, sector) == 0) /* READ_SINGLE_BLOCK */
		&& rcvr_datablock(buff, BLK_SIZE))
			count = 0;
	} else { /* Multiple block read */
		if (send_cmd(CMD18, sector) == 0) { /* READ_MULTIPLE_BLOCK */
			do {
				if (!rcvr_datablock(buff, BLK_SIZE))
					break;
				buff += BLK_SIZE;
			} while (--count);
